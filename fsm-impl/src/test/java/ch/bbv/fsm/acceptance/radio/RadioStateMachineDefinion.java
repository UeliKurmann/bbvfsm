...
public class RadioStateMachineDefinion extends AbstractStateMachineDefinition<RadioStateMachine, State, Event> {
	...
	private void define() {
		...
		in(State.Off).on(Event.TogglePower).goTo(State.On).execute(sm -> sm.logTransitionFromOffToOn())
				.onlyIf((sm, p) -> sm.isUserMode());
		in(State.Off).executeOnEntry((sm) -> sm.logOffEntry());
		in(State.Off).executeOnExit((sm) -> sm.logOffExit());

		in(State.On).on(Event.TogglePower).goTo(State.Off).execute(sm -> sm.logTransitionFromOnToOff());
		in(State.On).executeOnEntry((sm) -> sm.logOnEntry());
		in(State.On).executeOnExit((sm) -> sm.logOnExit());

		in(State.Off).on(Event.TogglePower).goTo(State.Maintenance).execute(sm -> sm.logTransitionOffToMaintenance())
				.onlyIf((sm, p) -> sm.isMaintenanceMode());

		in(State.Maintenance).on(Event.TogglePower).goTo(State.Off).execute(sm -> sm.logTransitionFromMaintenanceToOff());
	...
	private void defineOnBla() {
		...
		in(State.FM).on(Event.ToggleMode).goTo(State.AM).execute(sm -> sm.logTransitionFromFMToAM());
		in(State.FM).executeOnEntry((sm) -> sm.logFMEntry());
		in(State.FM).executeOnExit((sm) -> sm.logFMExit());

		in(State.AM).on(Event.ToggleMode).goTo(State.FM).execute(sm -> sm.logTransitionFromAMToFM());
	...
	private void defineAM() {
		...
		in(State.Play).on(Event.StationLost).goTo(State.AutoTune).execute(sm -> sm.logTransitionFromPlayToAutoTune());
		in(State.Play).executeOnEntry((sm) -> sm.logPlayEntry());
		in(State.Play).executeOnExit((sm) -> sm.logPlayExit());

		in(State.AutoTune).on(Event.StationFound).goTo(State.Play).execute(sm -> sm.logTransitionFromAutoTuneToPlay());
